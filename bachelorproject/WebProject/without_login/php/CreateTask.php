	...
	class CreateTask{
		...
		function CreateTask(){
			...
			if($this->checkSequenceTime()){
				...
				$sql = "INSERT INTO Task values (0, (Select UID FROM Users WHERE UID =".$_SESSION['UID']."), (SELECT VideoId from VideoInformations where active=1),  (SELECT sequence FROM VideoInformations WHERE active=1), '".$token."', '".$campaign."', 5, 0, 0)";
