...
$(document).on('click', '#changeEvent', function(){
	...
	var gameEvent = $('#Events option:selected').val();
	...
	if (gameEvent == '--') {
		...
		if (team == '--') {
			...
			if(video.currentTime < startTime){
				...
				$('#clearEvent').hide();
				$('#addEvent').show();
				updateObject(timeToSeconds(minutes, seconds, milliseconds), gameEvent, team);
				showDefault();
			}
		}
	}
...
function updateObject(time, gameEvent, team){
